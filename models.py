...
from layers.modules.roi_pooling import ROIPooling2dPytorch
...
class FlowNet2(nn.Module):
    ...
    def __init__(self, args, batchNorm=False, div_flow = 20.):
        ...
        self.POOLING_SIZE_flow = 25
        self.roi_pool_flow = ROIPooling2dPytorch(self.POOLING_SIZE_flow, 1.0)

        self.offset_pred_x_ = nn.Sequential(
            nn.Linear(625, 2048),
        ...
        self.offset_pred_y_ = nn.Sequential(
            nn.Linear(625, 2048),
            nn.ReLU(inplace=True),
            nn.Linear(2048, 2048),
            nn.ReLU(inplace=True),
        ...
        self.offset_pred_w_ = nn.Sequential(
            nn.Linear(625, 2048),
            nn.ReLU(inplace=True),
            nn.Linear(2048, 2048),
            nn.ReLU(inplace=True),
        ...
        self.offset_pred_h_ = nn.Sequential(
            nn.Linear(625, 2048),
            nn.ReLU(inplace=True),
            nn.Linear(2048, 2048),
            nn.ReLU(inplace=True),
    ...
    def forward(self, inputs=None, rois=None, flow=None, mode='flow'):
        if mode == 'flow':
            ...
            flownetfusion_flow = flow
            pooled_feat = self.roi_pool_flow(flownetfusion_flow, rois)
            pooled_feat_x = pooled_feat[:,0,:,:]#.unsqueeze(1)
            pooled_feat_y = pooled_feat[:,1,:,:]#.unsqueeze(1)

            pooled_feat_x = pooled_feat_x.view(pooled_feat_x.shape[0], 625)
            pooled_feat_y = pooled_feat_y.view(pooled_feat_y.shape[0], 625)

            bbox_pred_x = self.offset_pred_x_(pooled_feat_x)
            bbox_pred_y = self.offset_pred_y_(pooled_feat_y)
            bbox_pred_w = self.offset_pred_w_(pooled_feat_x)
            bbox_pred_h = self.offset_pred_h_(pooled_feat_y)

            bbox_pred = torch.cat((bbox_pred_x,bbox_pred_y,bbox_pred_w,bbox_pred_h),dim=1)
