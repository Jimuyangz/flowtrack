...
class _FPN(nn.Module):
    ...
    def _PyramidRoI_Feat(self, feat_maps, rois, im_info):
        ...
        if cfg.POOLING_MODE == 'align':
            ...
            for i, l in enumerate(range(2, 6)):
                ...
                idx_l = (roi_level == l).nonzero().squeeze()
                #print(idx_l.shape)
                if len(idx_l.shape)==0:
                    idx_l = torch.tensor([idx_l.item()]).cuda()
                box_to_levels.append(idx_l)
                scale = feat_maps[i].size(2) / im_info[0][0]
                # print(feat_maps[i].shape, rois[idx_l], scale)
                feat = self.RCNN_roi_align(feat_maps[i], rois[idx_l], scale)
                roi_pool_feats.append(feat)
            roi_pool_feat = torch.cat(roi_pool_feats, 0)
            #print(box_to_levels)
    ...
    def forward(self, im_data, im_info, rois_gt, gt_boxes, num_boxes):
        ...
        # print(rois.shape)
        # print(im_info)
        # print('rois_gt:',rois_gt)
        roi_pool_feat = self._PyramidRoI_Feat(mrcnn_feature_maps, rois_gt, im_info)
        ...
        rois_gt = rois_gt.view(batch_size, -1, rois_gt.size(1))
        ...
        # return rois_gt, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label
        return rois_gt, cls_prob, bbox_pred, 0, 0, RCNN_loss_cls, RCNN_loss_bbox, 0
