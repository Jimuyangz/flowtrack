...
if __name__ == '__main__':
    ...
    def inference(image_dir, model_flow, FPN):
        ...
        for t in range(num):
            ...
            if len(det_2) != 0:
                ...
                # print('cur T', T[:current_num, :t+1, :])
                inact_idx = np.where(T[:current_num, t+1, 4]!=t+1)[0]
                # print('inact id', inact_idx)
                inact_T = T[inact_idx, :, :]
                ...
                if t == 0:
                    ...
                    continue
                # print('unmatched size',len(det_2))
                # print('inact',inact_T.shape)
                for v in range(1,stop,step): # stop is the furthest to go, and step is interval
                    ...
                    idx_ori = inact_idx[v_idx].tolist() # track index
                    v_T = inact_T[v_idx, t-v, :4] # n x 4

                    images_ = torch.from_numpy(images_.astype(np.float32))
                    images_ = images_.unsqueeze(0).cuda()

                    rois_ = torch.tensor(v_T.astype(np.float32)).unsqueeze(0)
                    rois_ = rois_.cuda()

                    idx_ori = np.array(idx_ori)
                    
                    with torch.no_grad():
                        offset_pred_ = model_flow(inputs=images_, rois=rois_ , mode='full')
                        # print('offset',offset_pred_.shape)

                    offset_pred_ = offset_pred_.detach().cpu().numpy()
                    rois_BT_ = rois_.squeeze(0).detach().cpu().numpy()
                    ...
                    with torch.no_grad():
                        # print('b_pred_BT',b_pred_BT.shape)
                    ...
                    det_2 = np.delete(det_2, np.array(del_id), 0)
                    if len(det_2) == 0:
                        break
                # print('after bt', len(det_2))
                if len(det_2) != 0:
                    #not exist
                    for d in det_2:
                        # print('d', d)
                        temp = d[:4].tolist()
                        temp.append(t+1)
                        # print(temp.append(t+1))
                        # print(np.array(temp.append(t+1)))
                        T[current_num,t+1,:] = np.array(temp)
    ...
    #load_name = 'models/05/faster_rcnn_1_20_103.pth'
    #load_name = 'models/11/faster_rcnn_1_20_111.pth'
    #load_name = 'models/10/faster_rcnn_1_20_80.pth'
    #load_name = 'models/09/faster_rcnn_1_20_64.pth'
    #load_name = 'models/13/faster_rcnn_1_20_92.pth'
    load_name = 'models/04/faster_rcnn_1_20_130.pth'
    #load_name = 'models/02/faster_rcnn_1_20_74.pth'
    ...
    #image_dir = '/data/OPT4/MOT17/test_flowtrack/MOT17-06-DPM/img_input'
    image_dir = '/home/jimuyang/OPT4/MOT17/test_flowtrack/MOT17-03-DPM/img_input'
