...
def roi_pooling_2d(input, rois, output_size=(7, 7), spatial_scale=1.0):
    """Applies a 2D ROI pooling over an input signal composed of several input
...
def roi_pooling_2d_pytorch(input, rois, output_size=(7, 7), spatial_scale=1.0):#, img=None):
    ...
    for i in range(num_rois):
        ...
        im = input.narrow(0, im_idx, 1)[...,
                                        roi[2]:roi[4]+1, #(roi[4]+1)
                                        roi[1]:roi[3]+1] #(roi[3]+1)

        output.append(F.adaptive_avg_pool2d(im, output_size))

    # print(torch.cat(output, 0).shape)60,32,7,7
    return torch.cat(output, 0)#, img
